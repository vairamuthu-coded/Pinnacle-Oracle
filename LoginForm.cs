    ...
    public partial class LoginForm : Form
    {
        ...
        void logo()
        {
            try
            {
                ...
                dtCC = Utility.SQLQuery("SELECT LOGO AS EMPIMAGE FROM EDOCIMAGE WHERE IMGNAME = 'COMPLOGO' AND COMPANYID ='" + Class.Users.HUnitSub + "' ");
                ...
                {
                    Class.Users.HUnit = data[3].ToString();
                    dtCC = Utility.SQLQuery("SELECT LOGO AS EMPIMAGE FROM EDOCIMAGE WHERE IMGNAME = 'COMPLOGOID' AND COMPANYID ='" + data[3].ToString() + "' ");
                    if (dtCC.Rows.Count > 0)
                    {
                        foreach (DataRow myRow in dtCC.Rows)
                        {
            ...
            catch (Exception EX) { if (Class.Users.HUnitSub == "pssagfsample") { button1.BackgroundImage = Pinnacle.Properties.Resources.Anugraha_logo; } }
        ...
        private void combo_compcode_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            //    ErrProvider.SetError(combo_compcode, "Invalid CompCoode"); combo_compcode.Focus();
            //}
            //else
            //{
            //    e.Cancel = false;
        ...
        private void txt_password_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            //{
            //    e.Cancel = false;
            //    ErrProvider.SetError(txt_password, "");
            //}
        }
        ...
        private void LoginForm_Load(object sender, EventArgs e)
        {
            try
            {
                txtusername.Text = "ADMIN"; txt_password.Text = "ADMIN";
                systemuser = Environment.UserName;
                Class.Users.Intimation = "PAYROLL";
                Class.Users.HUserName = txtusername.Text;
...
//                DataTable dtCC = Utility.SQLQuery("select MAX(TO_NUMBER(A.SESSIONTIME)) AS SESSIONTIME from asptblusermas A join gtcompmast b on a.compcode=b.gtcompmastid where b.compcode='" + Class.Users.HCompcode + "' and  a.username='" + Class.Users.HUserName + "'"
//                );

//                Class.Users.LoginTime = Convert.ToInt32(dtCC.Rows[0]["SESSIONTIME"].ToString());

//                string sel10 = "SELECT X.SID,X.SERIAL#, X.OSUSER,X.LOGON_TIME,X.STATUS,X.PREV_EXEC_START ,X.LAST_CALL_ET,X.program FROM( SELECT A.SID,A.SERIAL#, A.OSUSER,A.LOGON_TIME,A.STATUS,A.PREV_EXEC_START ,A.LAST_CALL_ET,a.program FROM v$SESSION a where A.PROGRAM='Pinnacle.exe' UNION select A.SID,A.SERIAL#, A.OSUSER,A.LOGON_TIME,A.STATUS,A.PREV_EXEC_START ,A.LAST_CALL_ET,a.program FROM v$SESSION a where A.PROGRAM='Pinnacle.exe')X WHERE X.OSUSER='" + systemuser + "' ORDER BY 1";
...
//                        {
//                            Class.Users.UserTime = Convert.ToInt32("0" + dt.Rows[0]["SESSIONTIME"]);
//                            string up = "update asptblusermas a set a.SESSIONTIME='600'  where a.userid='" + dt.Rows[0]["userid"] + "' and a.compcode='" + dt.Rows[0]["gtcompmastid"] + "'";
