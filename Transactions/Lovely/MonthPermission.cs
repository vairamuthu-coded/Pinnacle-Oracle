...
{
    public partial class MonthPermission : Form,ToolStripAccess
    {
        private static MonthPermission _instance;
        public MonthPermission()
        {
            InitializeComponent();
        ...
        public static MonthPermission Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new MonthPermission();
      ...
      //  Report.Lovely.LovelySalarySlip1 rd = new Report.Lovely.LovelySalarySlip1();
        ...
        public void Finyearload()
        {
            try
            {
                string sel0 = "SELECT 0  GTFINANCIALYEARID, '' Finyear, '' CURRENTFINYR FROM DUAL UNION ALL SELECT DISTINCT  GTFINANCIALYEARID,  FINYR AS  Finyear,CURRENTFINYR FROM  GTFINANCIALYEAR  ORDER BY 2";//WHERE E.CURRENTFINYR = 'T'";SELECT 0  GTFINANCIALYEARID, '' Finyear, '' CURRENTFINYR FROM DUAL UNION ALL
        ...
        public void GridLoad()
        {
            int j = 0; listView1.Items.Clear(); DataTable dt = new DataTable();
                ...
                string sel0 = "SELECT  DISTINCT '' AS  ASPTBLPAYSLIPPERID, A.PAYPERIOD AS PAYMONTH,A.FINYEAR,D.COMPCODE,Z.STDT  FROM LOPPLHPAYROLL A   JOIN HREMPLOYMAST B ON A.EMPID=B.HREMPLOYMASTID  JOIN HREMPLOYDETAILS C ON C.HREMPLOYMASTID=B.HREMPLOYMASTID AND B.IDCARDNO=C.IDCARD AND C.IDACTIVE='YES' JOIN GTCOMPMAST D ON D.GTCOMPMASTID=B.COMPCODE   JOIN ASPTBLUSERMAS E ON E.COMPCODE=D.GTCOMPMASTID  AND E.COMPCODE=B.COMPCODE JOIN MONTHLYPAYFRQ   Z ON Z.PAYPERIOD = A.PAYPERIOD   where  A.FINYEAR LIKE'%" + combofinyear.Text + "%' AND   D.compcode='" + Class.Users.HCompcode + "'  and E.username='" + Class.Users.HUserName + "' order by 2";
        ...
        public void Saves()
        {
            ...
            try
            {
                ...
                if (combocompcode.Text != "" &&  combouser.Text != "" && listView2.Items.Count > 0)
                {
       ...
       private void GridLoad1()
        {
            ...
            string sel1 = " select  distinct A.asptblpayslipperID, A.PAYMONTH,A.FINYEAR ,D.COMPCODE   from asptblpayslipper a   JOIN GTCOMPMAST D ON D.GTCOMPMASTID=A.COMPCODE       join asptblusermas e on E.COMPCODE=D.GTCOMPMASTID  AND E.USERNAME=A.USERNAME where   D.compcode='" + combocompcode.Text + "' and    E.USERNAME='" + combouser.Text + "'    order by 3";
        ...
        {
            if (combocompcode.Text != "" && combouser.Text != "")
            {
                GridLoad1();
            }
            else
